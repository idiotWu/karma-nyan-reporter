...
function NyanCat(baseReporterDecorator, formatError, config) {
  var self = this;
  this.totalBrowserCount = config.browsers.length;
...
NyanCat.prototype.draw = function(appendOnly){
  if (this.options.minimal || this.numberOfBrowsers !== this.totalBrowserCount) return;
...
NyanCat.prototype.onSpecComplete = function(browser, result) {
  ...
  const all = testStats.success + testStats.failed + testStats.skipped;
  testStats.total = Math.max(testStats.total, all);
...
NyanCat.prototype.onRunComplete = function() {
  ...
  if (this.browserErrors.length) {
    ...
    this.drawUtil.fillWithNewlines(this.options.minimal ? this.options.numberOfRainbowLines - 1 : 0);
